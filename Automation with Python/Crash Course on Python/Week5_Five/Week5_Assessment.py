...
"""Question 1
Let’s test your knowledge of using dot notation to access methods and attributes in an object. Let’s say we have a
class called Birds. 'Birds' has two attributes: color and number. Birds also has a method called count() that counts
the number of birds (adds a value to number). Which of the following lines of code will correctly print the number
of birds? Keep in mind, the number of birds is 0 until they are counted!

a)  bluejay.number = 0  \n  print(bluejay.number)
b)  print(bluejay.number.count())
c)  bluejay.count()   \n   print(bluejay.number)     (ok)
d)  print(bluejay.number)

We must first call the count() method, which will populate the number attribute,
